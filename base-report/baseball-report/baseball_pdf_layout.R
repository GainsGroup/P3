...
# 4 Spider Plot 
print(radar_plot_athl, vp = vplayout(7:21, 12:26))  ## Make middle right 
...
print(drawtable(skater %>% dplyr::rename("Lateral: Skater"=metric), fill_col = 'Percentile', fill = dkgrey, width='fill'), vp = vplayout(14:20, 2:11), newpage=FALSE)
upper <- percentiles_page2 %>% filter(test_type=="Upper Extremity") %>% select(metric, Percentile)
print(drawtable(upper %>% dplyr::rename("Upper Extrem."=metric), fill_col = 'Percentile', fill = dkgrey, width='fill'), vp = vplayout(21:25, 2:11), newpage=FALSE)
