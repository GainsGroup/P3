...
print(drawtable(drop_jump %>% dplyr::rename("Vertical: Drop Jump"=metric), fill_col = 'Percentile', fill = dkgrey, width='fill'), vp = vplayout(9:16, 2:11), newpage=FALSE)
st_vert <- percentiles_page2 %>% filter(test_type=="Standing Vertical") %>% select(metric, Percentile)
print(drawtable(st_vert %>% dplyr::rename("Vertical: Standing"=metric), fill_col = 'Percentile', fill = dkgrey, width='fill'), vp = vplayout(17:24, 2:11), newpage=FALSE)
skater <- percentiles_page2 %>% filter(test_type=="1 Off Skater") %>% select(metric, Percentile)

# 6 Training Targets 
print(drawtext(paste0(rec_one), 'Training Targets', header = FALSE), vp = vplayout(22:24, 12:24))
print(drawtext(paste0(rec_two), title = NULL, header = FALSE), vp = vplayout(25:26, 12:24))
print(drawtext(paste0(rec_three), title = NULL, header = FALSE), vp = vplayout(27:28, 12:24))
