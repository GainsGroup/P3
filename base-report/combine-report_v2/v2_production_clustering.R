...
clusterplot <- function(athlete_label){
 print("doing cluster plot")
  ...
  annotate("text", x = .08, y = -.12, label = "Perimeter Plus", color = "black", angle = 35, fontface = "bold", size =labsize) + ## cluster 2
...
cluster_text <- data.frame(
    ...
    Description = c("Traditional Big: Size tends to be the dominant physical characteristic of athletes in this cluster. Both Height and Length considerably outpace the typical NBA athlete. Improved power output - either vertically or laterally - can push an athlete into the Big Plus category.",
                    "Perimeter Plus: Generally, athletes in this cluster possess an impressive mix of size for their position and power output. Force production in all planes is 'elite', and this quality is paired with above-average kinematic efficiency - suggesting that athletes in this cluster are able to take advantage of their endemic power.",
                    "Misc. Perimeter: Athletes in this cluster possess adequate Height and Length measures relative to the NBA population, but fail to pair these metrics with 'plus' movement skills. Both force production and kinematic variables grade-out at as below-average in all planes.",
                    ...
                    "Force Mover: Force production - in all planes of movement - separate this group from the other clusters. While these athletes do possess impressive power output - leading to elite traditional performance measures - they may lack kinematic efficiency.",
                    "Hyper-Athletic Guards: Athletes in this cluster tend to lack the typical height / length of an NBA athlete. However, they make up for these anthropometric measures with elite force production and kinematic efficiency. The rare athlete in this cluster with NBA-average height / length will possess a series of physical advantages."))
